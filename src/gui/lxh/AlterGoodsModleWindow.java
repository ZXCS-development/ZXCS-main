...
public class AlterGoodsModleWindow extends JDialog{
		GetAllGoodsDao goods_dao=null;
		...
		OldGoodsADDModelWindow old=null;
		int id=0;	
		public AlterGoodsModleWindow(OldGoodsADDModelWindow old){
			goods_dao=new GetAllGoodsDao();
			this.old=old;
			id=Integer.parseInt(AlterGoodsModleWindow.this.old.isSelectTable().get(0)+"");
			...
			tf_type.setText(goodsdao.getAllGoods2(id).get(0)+"");
			
			tf_number=new JTextField(16);
			tf_number.setText(id+"");
			
			tf_name=new JTextField(16);
			tf_name.setText(goodsdao.getAllGoods2(id).get(1)+"");
			
			tf_bar=new JTextField(16);
			tf_bar.setText(goodsdao.getAllGoods2(id).get(2)+"");
			
			tf_spec=new JTextField(16);
			tf_spec.setText(goodsdao.getAllGoods2(id).get(3)+"");
			
			tf_unit=new JTextField(16);
			tf_unit.setText(goodsdao.getAllGoods2(id).get(4)+"");
			
			tf_sum=new JTextField(16);
			tf_sum.setText(goodsdao.getAllGoods2(id).get(5)+"");
			
			tf_color=new JTextField(16);
			
			tf_inprice=new JTextField(16);
			tf_inprice.setText(goodsdao.getAllGoods2(id).get(6)+"");
			
			tf_outprice=new JTextField(16);
			tf_outprice.setText(goodsdao.getAllGoods2(id).get(7)+"");
			
			tf_bz=new JTextField(40);
			tf_bz.setText(goodsdao.getAllGoods2(id).get(8)+"");
			...
			btn_ok.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
						...
						goodsdao.AlterGoods(id, type, gname, goodsid, norms, unit, alertNum, in_price, sell_price, bz);
						if(AlterGoodsModleWindow.this.old.table_l.isShowing()){
							try{
								updaateTable(AlterGoodsModleWindow.this.old.model_l, AlterGoodsModleWindow.this.old.table_l, AlterGoodsModleWindow.this.old.vector_l);
								updaateTable(AlterGoodsModleWindow.this.old.model_c, AlterGoodsModleWindow.this.old.table_c, AlterGoodsModleWindow.this.old.vector_c);
								updaateTable(AlterGoodsModleWindow.this.old.model_r, AlterGoodsModleWindow.this.old.table_r, AlterGoodsModleWindow.this.old.vector_r);
							}catch (Exception e2) {
								
							}
						}
						AlterGoodsModleWindow.this.setVisible(false);
					} catch (Exception e2) {
		 ...
		 * 更新表
		 */
		public void updaateTable(DefaultTableModel medel,JTable table,Vector vector){
			medel=new DefaultTableModel(goods_dao.getAllGoods1(""),vector);
